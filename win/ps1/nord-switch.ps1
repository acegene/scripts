# Quick switching of nordvpn's connected country
#
# usage
#   * nord-switch.ps1
...
function _nord-switch {
    ...
    if ($country -ne $null) {
        ...
        if ($group -eq $null) {
            $group = 'United States'
            Write-Host "WARNING: state for country '$($state)' unrecognized, using '$($group)'"
        } else {
            Write-Host "INFO: state for country should switch from '$($state)' to '$($group)'"
        }
    }
