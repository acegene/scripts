...
class WrappedIndexableCallableTestCase(unittest.TestCase):
    ...
    def assert_containers_equal(self, lhs, rhs, s):
        if len(lhs) != 0:
            if str(lhs) != str(rhs):
                assert False, str(lhs) != str(rhs)
        for l, r in zip_equal(lhs, rhs):
            assert l == r, 'For ' + str(s) + ': ' + str(l) + ' != ' + str(r)
            assert l in rhs, 'For ' + str(s) + ': ' + str(l) + ' not in rhs'
        for l, r in zip_equal(reversed(lhs), reversed(rhs)):
            assert l == r, 'For ' + str(s) + ': ' + str(l) + ' != ' + str(r)
        length_lhs = len(lhs)
        for i in range(length_lhs):
            assert lhs[i] == rhs[i], 'For ' + str(s) + ': index: ' + str(i) + ': ' + str(lhs[i]) + ' != ' + str(rhs[i])
    ...
    def test__iterable_from_indexable(self):
        ...
        steps = [step for step in range(-expected_base[-1]-1,0)] + [step for step in range(1,expected_base[-1]+2)] + [None]
        slices = [slice(start, stop, step) for start in starts for stop in stops for step in steps]
        #### iterate over slices and test class
        for s1 in slices:
            actual_func = wrapped_indexable_callable.WrappedIndexableCallable(self.indexable_func, expected_base_length, s1)
            actual_list = wrapped_indexable_callable.WrappedIndexableCallable([0,1,2,3,4,5], expected_base_length, s1)
