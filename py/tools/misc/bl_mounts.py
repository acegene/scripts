...
if os.geteuid() != 0:
    ...
    # script = sys.argv[0]
    args = sys.argv[1:]
    subprocess.run(['sudo', 'python3'] + [os.path.abspath(__file__)] + args)
    sys.exit(0)
...
def _unmount(directory):
    subprocess.run(['sudo', 'umount', '-l', directory], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
def _bl_mount(partition, dir_mount, dir_bl):
    ...
    try:
        with open(f"{dir_mount}/README.md", 'r', encoding='utf-8') as f:
...
def main():
    ...
    input_str = input(f"PROMPT: Accept found mountable paths above? (y/n): ").lower()
    if input_str != "y":
        ...
        while True:
            change_indices_str = input(f"PROMPT: Give space delimited integers for for the above indices you wish to change: ")
            try:
        ...
        for k in change_keys:
            ...
            mountable_paths_to_mount_points
    ...
    if len(prompt_mountable_paths) > 0:
        while True:
            ...
            if len(input_mount_integers) != len(prompt_mountable_paths):
                print(f"ERROR: len(input_mount_integers) != len(prompt_mountable_paths): {len(input_mount_integers)} != {len(prompt_mountable_paths)}")
                continue
            if any([_make_mount_names_from_index(i) in no_prompt_mountable_paths_to_mount_points.values() for i in input_mount_integers]):
    ...
    input_str = input(f"PROMPT: Continue? (y/n): ").lower()
    if input_str != "y":
    ...
    for path_to_mount in paths_to_mount:
        ...
        while True:
            ...
            if bl_mount_ec == 0:
                ...
                print(f"ERROR: seems encrypt passed but the readme could not be read, create one then try again")
                input('PROMPT: press enter when ready to continue')
            else:
                print(f"ERROR: failed decrypt, try again")
